...
params <- gen.params.ffms_base(ncol(df.train) - 2)
...
set.seed(121)
result1 <- ffms(formula = cens ~ 1 + .,data = df.train[,-1], params = params, P = 5,
                transforms = transforms, method = "ffms_base",
...
set.seed(122)
result2 <- ffms(formula = cens ~ 1 + .,data = df.train[,-1], params = params,
                method = "fms.parallel", runs = 4, cores = 4,
                family = "custom", loglik.pi = surv.pseudo.loglik, 
...
result3 <- ffms(formula = cens ~ 1 + .,data = df.train[,-1], params = params, probs = probs, P = 10,
                transforms = transforms, method = "ffms.parallel", runs = 4, cores = 4,
                family = "custom", loglik.pi = surv.pseudo.loglik, 
                model_prior = list(r = 0.5), extra_params = list(time = time))
...
probs$gen <- c(1,1,1,1)
params$feat$D <- 5
result4 <- ffms(formula = cens ~ 1 + .,data = df.train[,-1], params = params, probs = probs,P = 20, 
                transforms = transforms, method = "ffms.parallel", runs = 4, cores = 4,
                family = "custom", loglik.pi = surv.pseudo.loglik, 
                model_prior = list(r = 0.5), extra_params = list(time = time))
