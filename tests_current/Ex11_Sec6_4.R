...
set.seed(100001)
tic()
result1 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 2,
                transforms = transforms, params = params, method = "ffms_base",
...
# regular analysis
tic()
result2 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 2,
                transforms = transforms, params = params, method = "ffms_base", 
...
tic()
result_parallel_1 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 3,
                          transforms = transforms, params = params, 
                          method = "ffms.parallel", runs = 2, cores = 2,
                          family = "custom", loglik.pi = logistic.posterior.bic.irlssgd, 
                          model_prior = list(r = r, subs = 0.01), sub  = T)
time3 <- toc() 
...
tic()
result_parallel_2 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 3, 
...
set.seed(100005)
tic()
result_parallel_long_1 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 10,
                          transforms = transforms, params = params, N = 500,
                          method = "ffms.parallel", runs = 4, cores = 4,
                          family = "custom", loglik.pi = logistic.posterior.bic.irlssgd, 
                          model_prior = list(r = r, subs = 0.01), sub  = T)
...
tic()
result_parallel_long_2 <- ffms(formula = HeartDiseaseorAttack ~ 1 + ., data = df, P = 10, 
                          transforms = transforms, params = params, family = "binomial", 
                          method = "ffms.parallel", runs = 4, cores = 4, N = 500,
                          model_prior = list(r = r),
                          beta_prior = list(type = "Jeffreys-BIC"))
time6 <- toc() 
