...
df$y = rnorm(n =n, mean = mu,sd = sigma)


transforms <- c("p0","p2","p3","p05","pm05","pm1","pm2","p0p0","p0p05","p0p1","p0p2","p0p3","p0pm05","p0pm1","p0pm2", "p0pm05")
#transforms <- c("p0","p05","pm05","pm1","p0pm05")
...
result <- ffms(data = df, method = "ffms_base", transforms = transforms, 
                 probs = probs, params = params, prob_gen = c(0, 1, 0, 1))
summary(result)
...
result_parallel <- ffms(data = df, method = "ffms.parallel", transforms = transforms, 
                          probs = probs, params = params, prob_gen = c(0, 1, 0, 1), P=100,runs = 8, cores = 8)
summary(result_parallel, tol = 0.05)
summary(result_parallel, top_N = 20, tol = 0.05)
...
result_parallel2 <- ffms(data = df, method = "ffms.parallel", transforms = transforms, 
                           probs = probs, params = params,prob_gen = c(0, 1, 0, 1), P=100, N=1000, N.final=2000, 
                           runs = 16, cores = 8,)
...
set.seed(100)
result_parallel3 <- ffms(data = df, method = "ffms.parallel", beta_prior = list(type = "Jeffreys-BIC"), transforms = transforms, 
                         probs = probs, params = params, prob_gen = c(0, 1, 0, 1), P=100, N=1000, N.final=2000, 
                         runs = 8, cores = 4,)
