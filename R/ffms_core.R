...
#' Main Algorithm for FFMS (Genetically Modified FMS)
#'
...
#' @param N.final The number of FMS iterations performed for the final population. Per default one has N.final = N, but for practical applications, a much larger value (e.g., N.final = 1000) is recommended. Increasing N.final is particularly important if predictions and inferences are based solely on the last population.
#' @param probs A list of various probability vectors used by FFMS, generated by \code{gen.probs.ffms_base}. 
...
#' @param params A list of various parameter vectors used by FFMS, generated by \code{gen.params.ffms_base}.
...
#'   Alternative estimation strategies ("deep" and "random") are implemented in \pkg{FFMS}.
...
#' \item{mc.models}{All models accepted by fms_base per population.}
#' \item{populations}{All features per population.}
#' \item{sic.probs}{Marginal feature probabilities per population.}
...
ffms_base <- function (
  ...
  for (p in seq_len(P)) {
    ...
    # Run FMS over the population
    if (verbose) cat(paste("Population", p, "begin."))
    sic_res <- sic_optimize.loop(data.t, complex, loglik.pi, model.cur, N.this, probs, params, sub, verbose)
    if (verbose) cat(paste("\nPopulation", p, "done.\n"))

    # Add the models visited in the current population to the model list
    models[[p]] <- sic_res$models
    mc.models[[p]] <- sic_res$mc.models
    # Store marginal likelihoods for current features
    sic.probs[[p]] <- sic_res$sic.probs
    # Store marginal likelihoods for the visited models
    model.probs[[p]] <- sic_res$model.probs
    # Store indices for which the marginal likelihoods for the visited models refer to
    model.probs.idx[[p]] <- sic_res$model.probs.idx
    # Store best marginal model probability for current population
    best.margs[[p]] <- sic_res$best.crit
    # Store the accepted number of steps for the current population
    accept[[p]] <- sic_res$accept
    # Print the marginal posterior distribution of the features after FMS
    if (verbose) {
      cat(paste("\rCurrent best crit:", sic_res$best.crit, "\n"))
      cat("Feature importance:\n")
      print_dist(sic.probs[[p]], sapply(S[[p]], print.feature, labels = labels, round = 2), probs$filter)
    }
    ...
    if (p != P) {
      S[[p + 1]] <- ffms_base.transition(S[[p]], F.0, data, loglik.alpha, sic.probs[[1]], sic.probs[[p]], labels, probs, params$feat, verbose)
  ...
  results <- list(
    ...
    mc.models = mc.models,             # Only fms_base models per population
    populations = S,                   # All features per population
    sic.probs = sic.probs,           # Marginal feature probabilities per population
  ...
  attr(results, "class") <- "ffms_base"
  return(results)
}


#' Subalgorithm for Generating a New Population of Features in FFMS (Genetically Modified FMS)
#'
...
#' @param sic.probs The marginal inclusion probabilities of the current features
#' @param sic.probs.F.0 The marginal inclusion probabilities of the initial population of features
...
#' @noRd
#' 
ffms_base.transition <- function (S.t, F.0, data, loglik.alpha, sic.probs.F.0, sic.probs, labels, probs, params, verbose = TRUE) {
  # Sample which features to keep based on marginal inclusion below probs$filter
  feats.keep <- as.logical(rbinom(n = length(sic.probs), size = 1, prob = pmin(sic.probs / probs$filter, 1)))

  # Always keep original covariates if that setting is on
  if (params$keep.org) {
    if (params$prel.filter > 0) {
      # Do preliminary filtering if turned on
      feats.keep[(seq_along(F.0))[sic.probs.F.0 > params$prel.filter]] <- T
  ...
  sic.probs.use <- c(rep(params$eps, length(F.0)), pmin(pmax(sic.probs, params$eps), (1-params$eps)))
  ...
  for (i in feats.replace) {
    ...
    if (prev.size > params$pop.max) {
      ...
      S.t[[i]] <- gen.feature(c(F.0, S.t), sic.probs.use, data, loglik.alpha, probs, length(F.0), params, verbose)
      ...
      feats.keep[i] <- T
      sic.probs.use[i] <- mean(sic.probs.use)
    }
  }
  ...
  if (length(S.t) < params$pop.max) {
    for (i in (length(S.t)+1):params$pop.max) {
      ...
      sic.probs.use <- c(sic.probs.use, params$eps)
    }
  }
  return(S.t)
}
