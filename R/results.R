...
#' if data is supplied it should be of the same form as is required by ffms_base, i.e. with both x, y and an intercept.
#'
...
#'
#' @param result A fitted model object (e.g., from \code{fms_base}, \code{ffms_base}, or related classes) containing the summary statistics and marginal probabilities.
...
#'
#' # Assume 'result' is a fitted object from ffms_base or fms_base
#' result <- fms_base(cbind(y,x))  
#'
#' # Get the MPM
...
get.mpm.model <- function(result, y, x, labels = F, family = "gaussian", loglik.pi = gaussian.loglik, params = NULL) {
  ...
  complex <- if (length(features) > 0) {
    complex.features(features)
  } else {
    list(width = numeric(0), oc = numeric(0), depth = numeric(0))
  }
  
  coefs <- loglik.pi(y = y, x = precalc$x, model = rep(TRUE, length(features) + result$fixed), complex = complex, mlpost_params = params)$coefs
...
#'
#' @param result An object of class \code{fms_base}, \code{fms.parallel}, \code{ffms_base}, or \code{ffms_merged}, 
...
#' \describe{
#'   \item{\code{fms_base}}{Selects the top model from a single FMS run.}
#'   \item{\code{fms.parallel}}{Identifies the best chain, then selects the best model from that chain.}
#'   \item{\code{ffms_base}}{Selects the best population and model within that population.}
#'   \item{\code{ffms_merged}}{Finds the best chain and population before extracting the top model.}
#' }
#'
#' @examples
#' data(exoplanet)
...
get.best.model.fms_base <- function(result, labels = FALSE, ...) {
  # Input validation
  stopifnot(
    "result must be of class 'fms_base'" = inherits(result, "fms_base"),
...
get.best.model.fms_parallel <- function(result, labels = FALSE, ...) {
  # Input validation
  stopifnot(
    "result must be of class 'fms.parallel'" = inherits(result, "fms_parallel"),
...
get.best.model.ffms_base <- function(result, labels = FALSE, ...) {
  # Input validation
  stopifnot(
    "result must be of class 'ffms_base'" = inherits(result, "ffms_base"),
    "result must contain models" = !is.null(result$models) && length(result$models) > 0,
    "result must contain populations" = !is.null(result$populations) && length(result$populations) > 0,
...
get.best.model.ffms_merged <- function(result, labels = FALSE, ...) {
  # Input validation
  stopifnot(
    "result must be of class 'ffms_merged'" = inherits(result, "ffms_merged"),
...
plot.fms_parallel <- function (x, count = "all", ...) {
  merged <- merge_fms.parallel(x)
  marg.prob.plot(merged$features, merged$marg.probs, count)
}

merge_fms.parallel <- function (x) {
